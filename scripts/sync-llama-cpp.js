...
console.log('  4. Arch drift gate: node scripts/create-dl-pack.js --check-archs');
console.log('     (ggml-cuda\'s arch list can move with a sync; scripts/dl-archs.js must mirror it)');
console.log('');
console.log('[sync-llama-cpp] The CUDA backend pack is keyed by lloyal.node VERSION, not by this tag: the next');
console.log('[sync-llama-cpp] v* tag builds, gates and publishes a pack for it (release.yml build-dl-flavor),');
console.log('[sync-llama-cpp] provided the binaries-channel service token is configured on the repo.');
